...
import { IUserRepository, TYPES } from "../types/index";
import { container } from "../../inversify.config";
import speakeasy from "speakeasy";

export const verify2fa = async function (req: Request, res: Response, next: NextFunction) {
...
export const generate2faQr = async function (req: Request, res: Response) {
  const { id } = req.currentUser;
  const userRepository = container.get<IUserRepository>(TYPES.IUserRepository);
...
export const enable2fa = async function (req: Request, res: Response, next: NextFunction) {
  const { id } = req.currentUser;
  const { token } = req.body;
  const userRepository = container.get<IUserRepository>(TYPES.IUserRepository);
  const user = await userRepository.findOne({ id });
...
export const disable2fa = async function (req: Request, res: Response, next: NextFunction) {
  const { id } = req.currentUser;
  const { token } = req.body;
  const userRepository = container.get<IUserRepository>(TYPES.IUserRepository);
  const user = await userRepository.findOne({ id });
