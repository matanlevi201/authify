...
import { IEmailRepository, IEmailService, IUserRepository, TYPES, UserPayload } from "../types/index";
...
export const resetPassword = async function (req: Request, res: Response) {
  ...
  try {
    payload = jwt.verify(resetToken as string, env.RESET_JWT_KEY) as UserPayload;
  } catch (error) {
