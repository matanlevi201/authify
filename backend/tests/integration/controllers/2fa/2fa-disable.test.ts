...
describe(`DELETE ${BASE_URL}`, () => {
  it("returns 401 when user is not signed in", async () => {
    await request(app).delete(`${BASE_URL}`).send().expect(401);
  });
  ...
  it("returns 200 when 2fa disabled", async () => {
    ...
    const twoFADisabledUser = await userRepository.findOne({ id: user.id });
    expect(twoFADisabledUser.is2FAEnabled).toBe(false);
