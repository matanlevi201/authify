...
describe(`PUT ${BASE_URL}/reset`, () => {
  ...
  it("returns 403 when token is not valid", async () => {
    await request(app)
      .put(`${BASE_URL}/reset?resetToken=invalid_token`)
      .send({ password: "123456789" })
      .expect(403);
  });
  ...
  it("returns 403 when token is expired", async () => {
    ...
    await request(app)
      .put(`${BASE_URL}/reset?resetToken=${expiredResetToken}`)
      .send({ password: "123456789" })
      .expect(403);
  });
  ...
  it("returns 406 when provided password is invalid", async () => {
    ...
    await request(app)
      .put(`${BASE_URL}/reset?resetToken=${resetToken}`)
      .send({ password: "" })
      .expect(406);
    await request(app)
      .put(`${BASE_URL}/reset?resetToken=${resetToken}`)
  ...
  it("returns 200 when password was reset successfully", async () => {
    ...
    const isPasswordUpdated = await Password.compare(
