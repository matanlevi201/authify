...
describe(`POST ${BASE_URL}/sessions`, () => {
  ...
  it("returns 406 when email is invalid", async () => {
    await request(app)
      .post(`${BASE_URL}/sessions`)
      .send({ password: "Aa@@123456" })
      .expect(406);
    await request(app)
      .post(`${BASE_URL}/sessions`)
      .send({ email: "", password: "Aa@@123456" })
      .expect(406);
    await request(app)
      .post(`${BASE_URL}/sessions`)
      .send({ email: 123, password: "Aa@@123456" })
      .expect(406);
    await request(app)
      .post(`${BASE_URL}/sessions`)
      .send({ email: [], password: "Aa@@123456" })
    ...
    await request(app)
      .post(`${BASE_URL}/sessions`)
      .send({ email: "testtest.com", password: "Aa@@123456" })
      .expect(406);
  });
  ...
  it("returns 400 when credentials are invalid", async () => {
    ...
    await userRepository.create({
      email: payload.email,
      password: await Password.toHash(payload.password),
    });
    await request(app)
      .post(`${BASE_URL}/sessions`)
  ...
  it("returns 200 when user signed in with 2fa is enabled", async () => {
    ...
    expect(accessToken).toBeDefined();
    const signedInUser = jwt.verify(
      accessToken,
      process.env.TEMP_2FA_JWT_KEY as string
    ) as UserPayload;
